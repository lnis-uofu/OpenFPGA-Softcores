...
all physical block information, then stored in a ``Block`` object.

>>> rpt = VprNetParser("<net-filename>")
>>> rpt.find_block("a[0]") # return the Block object containing the point 'a[0]'
"""
...
class Block:
    """Block object for easier access to block properties.

    Attributes:
        direction (str) : Direction of the signal (input/output).
        hierarchy (list): Instance hierarchy to retrace the signal depth.
        id        (int) : Block identifier used to locate the block in the VPR grid.
        name      (str) : Block name used by the post-BLIF design.
        pin_name  (str) : Pin name of the mapped instance.
        pin_numb  (int) : Pin number of the physical block, refering to a bus type.
        point     (str) : Full startpoint or endpoint name used in the timing report.
        port_name (str) : Port name of the physical block.
        type      (str) : Block type (`clb`, `memory`, `dsp` ...).
    
    Args:
        point_name (str): Specific point name of the block.
    """
...
class VprNetParser(object):
    ...
    trace the block properties, as defined in the ``Block`` object.
    ...
    def find_block(self, point_name):
        """Return the ``Block`` object of the point name else `None`.

        .. note::
            This method calls a recursive private method to search into each
            sub-blocks. Please avoid using this method for iterative searches.
