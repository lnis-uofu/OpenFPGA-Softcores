...
def print_statistics(path, stream=sys.stdout):
    ...
    print("Arrival time   : {arrival_time:.2f}".format(**data), file=stream)
    print("Path time      : {path_time:.2f} (net: {net_time:.2f}, pb: {pb_time:.2f})".format(**data), file=stream)
    print("Distance ratio : {ratio_dist:.2f} (manhattan: {manhattan_dist}, pb-to-pb: {pb2pb_dist})".format(**data), file=stream)

## Print a given path using a table format
def print_path(path, stream=sys.stdout):
    # path description
    print(f"# Path {path.id}", file=stream)
    print(f"Startpoint: {path.startpoint}", file=stream)
    print(f"Endpoint  : {path.endpoint}", file=stream)
    ...
    headers = [
        ...
        ('pb_type'  , "PB Type"),   # physical block type
        ('pb_direct', "Pin"),       # pin direction (in/out)
        ('pb_id'    , "Block"),     # block identifier
        ('coords'   , "Coords"),    # block coordinates
    ]
